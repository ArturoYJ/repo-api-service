...
import { loginSchema, verifyOtpSchema } from '../modules/auth/schemas/auth.schema';
...
authRouter.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const preAuth = await AuthService.login(email, password);

    res.cookie('pre_auth_token', preAuth.preToken, { ...COOKIE_OPTS, maxAge: 5 * 60 * 1000 });
...
authRouter.post('/verify-otp', async (req: Request, res: Response) => {
  try {
    ...
    const resultado = verifyOtpSchema.safeParse({ preToken, code: req.body.code });
    if (!resultado.success) {
      res.status(400).json({
    ...
    const { token, usuario } = await AuthService.verifyOtp(resultado.data.preToken, resultado.data.code);

    res.clearCookie('pre_auth_token', { path: '/' });
    res.cookie('auth_token', token, { ...COOKIE_OPTS, maxAge: 60 * 60 * 24 * 1000 });
